 ...
 */
export class UserBatchDeleteEventHandler extends BaseRedisEventHandler {
    constructor(
        private readonly pgPool: Pool,
        private readonly redisClient: RedisClientType<any, any>,
    ) {
        super('UserBatchDeleteEventHandler');
    }

    async handle(message: string): Promise<void> {
        this.logger.log(`Received user.batchDelete event: ${message}`);

        const userIdsToDelete: string[] = ['f3b488b2-c031-7034-6763-e88b46131614',
            '8394b812-60b1-70ee-5897-555e022f8d51', '53a4c842-90f1-70bf-6b94-2a2dcae2cad4',
            '33347892-2041-70b3-59fe-5d171cc4d192', '03048842-f051-70c5-7e11-ec18637dbaaf',
            'c3445802-9041-70e9-149a-bf622e4be5af', '2354b8a2-5011-700a-7551-87517cff2c34',
            'e394b8d2-3071-70b1-10c5-850180daf0eb', 'c3346872-d081-70f7-251d-245ffef7cbde',
            '036418f2-f061-7003-7063-0ae216d0d53a', 'f3d428e2-6001-706c-f711-f44675e313bf'
        ]
        ...
        try {
         ...
         WHERE id = $1`,
                ...
                this.logger.log(
                    `User deletion cleanup completed successfully for user ${userId}`,
                );
            }
        } catch (error) {
            await client.query('ROLLBACK');
            this.logger.error(`Error handling user.batchDelete event for users ${userIdsToDelete}:`, error);
            throw error;
        } finally {
            client.release();
        }
